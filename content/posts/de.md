...
<p class="sub">오른쪽 키보드보다 팜레스트가 먼저 도착한 직후 찍은 사진</p>

## 난관

무슨 생각으로 한 손 키보드의 팜레스트를 저렇게 경사지게 만든 건지 일반인인 나는 감히 알 수 없었지만 중요한 건 이전보다 손목이 더 아팠다는 것이다. 그래서 든 생각이 모니터 암을 달았을 때처럼 경화제를 평평하게 붙여서 양쪽 높이를 맞추는 것이었다. 그러나 에폭시 퍼티(경화제)의 가격이 만만치 않았고 그 정도로 높은 내구성이 필요한 것도 아니었기 때문에 좀 더 저렴한 폴리머 클레이라는 재료를 사용했다.

![](/images/policlay.png)

<p class="sub">25g에 500원 하는 폴리머클레이. 실온에서 굳지 않지만 열을 가하면 딱딱하게 굳는다</p>
...
한 손 게이밍 키보드에는 윈도우 버튼이 없다. 배그하다가 윈도우 버튼 누르지 말라고 빼버린 것이다. 위에서 언급했다시피 나는 마우스를 사용하지 않기 때문에 모니터 간 창 이동, 창 나누기 등등 윈도우 버튼을 필두로 하는 조합 명령에 의존적이었다.

이 주일 정도 지났을까, 윈도우키의 빈자리가 점점 커질 무렵, 우연히 쿠팡 회원은 로켓배송으로 구매한 상품을 한 달 안에 조건없이 반품해준다는 안내를 읽었다. 혹시나 하는 마음에 고객센터에 커터칼로 난도질한 자국이랑 찰흙 같은 게 잔뜩 붙어있는 키보드도 반품이 가능한지 문의했는데 가능하다는 답변을 받았다. 당연히 거절할 줄 알았는데 가능하다는 답변을 받자 잠시의 망설임도 없이 싹 다 반품하고 분리형 키보드를 사기로 결심했다.
...
MD770 RGB를 구매해서 약 3개월 정도 사용하고 있다. RGB감성을 싫어해서 평소에는 기능을 아예 꺼놓지만, 키보드 매크로를 설정하거나 매핑을 바꿀 때 RGB가 제공해주는 도움 표시가 유용하다.
